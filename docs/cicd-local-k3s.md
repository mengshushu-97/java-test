...
```text
mvn test
docker build -t harbor.local/test/java-test:$GITHUB_SHA .
docker push harbor.local/test/java-test:$GITHUB_SHA
...
```text
mvn test
docker build -t harbor.local/prod/java-test:$GITHUB_SHA .
docker push harbor.local/prod/java-test:$GITHUB_SHA
...
kubectl get applications -n argocd | grep java-test
```
...
```bash
kubectl rollout undo deployment/java-test -n test
kubectl rollout undo deployment/java-test -n prod
```
