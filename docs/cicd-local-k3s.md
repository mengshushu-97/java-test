...
```text
mvn test
docker build -t java-test:$GITHUB_SHA .
docker save java-test:$GITHUB_SHA
sudo k3s ctr -n k8s.io images import /tmp/java-test-$GITHUB_SHA.tar
kubectl apply -f k8s/test
kubectl -n test set image deployment/java-test java-test=java-test:$GITHUB_SHA
kubectl -n test rollout status deployment/java-test
```
...
kubectl -n prod set image deployment/java-test java-test=java-test:$GITHUB_SHA
kubectl -n prod rollout status deployment/java-test
```
...
- 不建议让 Argo CD 对这个应用开启 automated self-heal，否则可能把 Actions 设置的镜像 tag 回滚到 YAML 中的 `java-test:local`。
